...
class ExhibitorScraper:
    ...
    def find_exhibitor_list_url(self):
        ...
        リンクテキストに「出展企業一覧」「出展企業」「出展社一覧」などのキーワードを探す。
        """
    ...
    def find_exhibitor_links(self):
        """
        出展企業一覧ページから各企業の詳細ページへのリンクを取得する。
        サイト構造に合わせて、複数のセレクタパターンでリンクを抽出する。
        """
        exhibitor_links = []
        soup = self.get_soup(self.base_url)
        if not soup:
    ...
    def find_homepage_url(self, soup):
        """
        出展企業詳細ページ内から、企業のホームページURLを抽出する。
        リンクテキストに「ホームページ」や「公式サイト」などのキーワードが含まれるものを対象とする。
        """
    ...
    def extract_company_info(self, url):
        """
        出展企業詳細ページから情報を抽出する。
        また、企業ホームページURLが見つかった場合はそのページから従業員数、資本金情報を上書きする。
        """
        ...
        # ここでは一旦出展社社員数、出展社資本金、出展物については、
        # 出展企業詳細ページ内のテキストから抽出を試みる（必要に応じて調整）
        employee_patterns = ["社員数", "従業員数", "employees", "staff"]
        capital_patterns = ["資本金", "capital"]
        ...
        if homepage_url:
            ...
            # 企業ホームページから従業員数、資本金情報を取得して上書き（情報が取得できた場合）
    ...
    def scrape_exhibitors_from_top(self):
        """
        トップページ内に直接記載されている出展企業情報を抽出する。
        ※サイトによりセクションの構造は異なるため、セレクタは適宜調整してください。
        """
        soup = self.get_soup(self.base_url)
        if not soup:
            return []
